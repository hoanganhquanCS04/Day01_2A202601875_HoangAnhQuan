...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = (f"{'Prompt':<40} | {'GPT-4o Response':<40} | "
              f"{'Mini Response':<40} | {'GPT-4o Latency':<15} | {'Mini Latency':<15}")
    lines = [header, "-" * len(header)]
    for r in results:
        lines.append(
            f"{trunc(r['prompt']):<40} | {trunc(r['gpt4o_response']):<40} | "
            f"{trunc(r['mini_response']):<40} | {r['gpt4o_latency']:<15.2f} | "
            f"{r['mini_latency']:<15.2f}"
        )
    return "\n".join(lines)
